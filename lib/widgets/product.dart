...
class _ProductImageState extends State<ProductImage> {
  ...
  String oldPrice;
  _ProductImageState(this.productImage, this.productName, this.productPrice, this.productUnit,
      this.productCurrency, this.shopName, this.oldPrice);
  ...
  getRate() async {
    ...
    if (!cachedshops.containsKey(shopName)) {
      ...
      if (prefs.getString("cached_shops") != null && prefs.getString("caching_date") != null) {
          ...
          shopinfo = Firestore.instance
            ...
            (value) {
              if (value.documents.length > 0) {
                ...
                prefs.setString('cached_shops', json.encode(cachedshops));
                prefs.setString('caching_date', DateTime.now().toString());
                print('new rate is ' + value.documents[0].data['rate'].toString());
  ...
  Widget buildProductPrice() {
    // oldPrice = od
    var newOldPrice = (double.parse(oldPrice) * (rate != null ? rate : 1)).toInt().toString() + "L.L.";
                            ...
                            if (productPrice != null)
                              Text(
                                (double.parse(productPrice.toString()) *
                                            (rate != null ? double.parse(rate.toString()) : 1))
                                        .toInt()
                                        .toString() +
                                    "L.L.",
                                textAlign: TextAlign.left,
