...
class _ShopPageState extends State<ShopPage> {
  ...
  Future getcategories() async {
    ...
    type = await Firestore.instance.collection("types").document(shoptype.toLowerCase()).get();

    print("loaded and chosen cat is $chosenCategory and sub $chosenSubcategory");
    return chosenCategory;
  }
                                      ...
                                      child: Padding(
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            Row(
                                                mainAxisAlignment: MainAxisAlignment.start,
                                                children: type['categories'].keys.map<Widget>((entry) {
                                                  first++;
                                                  if (widget.data['categories'] != null)
                                                    return Visibility(
                                                      visible: widget.data['categories'].contains(entry),
                                                      child: Padding(
                                                        padding: first == 1
                      ...
                      builder: (context, snapshot) {
                        switch (snapshot.connectionState) {
                          ...
                          default:
                            if (snapshot.hasError)
                              return Text('Error: ${snapshot.error}');
                            else if (type != null && snapshot.connectionState == ConnectionState.done)
                              return Column(
                                children: [
                                  Column(
                                      children: type['categories'].keys.map<Widget>((entry) {
                                          ...
                                          child: Padding(
                                            ...
                                            child: Row(
                                                children: List<Widget>.generate(
                                                    type['categories'][entry].length, (int index) {
                                              return Visibility(
                                                visible: entry == chosenCategory &&
                                                        widget.data['subcategories']
                                                            .contains(type['categories'][entry][index])
                                                    ? true
                                                    : false,
                                                child: Padding(
                                                  padding: const EdgeInsets.only(
                                                      right: 0.0, bottom: 10, top: 0, left: 0),
                                                  child: GestureDetector(
                                                    onTap: () {
                                                      setSubCategory(type['categories'][entry][index]);
                                                    },
                                                    child: Container(
                                                        height: 50,
                                                        width: 110,
                                  ...
                                  Visibility(
                                    ...
                                    child: SizedBox(
                                      height: 200,
                                      child:
                                          Center(child: Image.asset("assets/images/loading.gif", height: 30)),
                                    ),
                                  ),
                                  Visibility(
                                    visible:
                                        !hideeverything && chosenCategory != '' && chosenSubcategory != ''
                                            ? true
                                            : false,
                                    child: Padding(
                                        padding:
                                        ...
                                        child: StreamBuilder(
                                          stream: Firestore.instance
                                              ...
                                              .where('category', isEqualTo: chosenCategory)
                                              .where('subcategory', isEqualTo: chosenSubcategory)
                                              .snapshots(),
                                          builder: (context, snapshot) {
                                            if (snapshot.hasData) {
